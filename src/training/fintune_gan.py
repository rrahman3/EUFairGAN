...
def train_esrgan(cnn_model, generator, esrgan_discriminator, gender_discriminator, num_epochs=25):
    ...
    optimizer_g = optim.Adam(generator.model.parameters(), lr=0.0001, betas=(0.5, 0.999))
    optimizer_d = optim.Adam(esrgan_discriminator.parameters(), lr=0.0001, betas=(0.5, 0.999))
    ...
    for epoch in range(num_epochs):
        ...
        for step, (lr_images, hr_images, gender_labels, ages) in enumerate(celeba_loader.train_dataloader):
            ...
            g_loss = esrgan_loss + 0.0001 * g_adversarial_loss
            ...
            print(f'step [{step+1}], d_loss: {d_loss.item():.8f}, g_loss: {g_loss.item():.8f}')
            print(f"ESRGAN train losses: {esrgan_loss.item()}, {g_adversarial_loss.item()}")
            # break
...
if __name__=="__main__":
    base_results_folder = celebAConfig.finetune_model_saved_pth
    loss_saved_file_name = utils.get_directory_to_save_file(folder_name=celebAConfig.celeba_results_folder, 
                                                            file_name=celebAConfig.celeba_esrgan_finetune_loss_log_filename, type="results")
    ...
    celeba_loader = CelebADataLoader(
                            ...
                            frac=celebAConfig.finetune_training_data_frac, 
